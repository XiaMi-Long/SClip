...
export function useSwiper() {
  ...
  const swiperBallHandler = reactive({
      ...
      switch (keyCode) {
        ...
        case 69:
          nextTick(() => {
            ...
            const activeIndex =
              swiperState.value.swiperInstance!.activeIndex * ITEMS_PER_PAGE + currentIndex
    ...
    validateSwiperIndexIsHas: (newIndex: number) => {
      if (swiperState.value.swiperInstance) {
        const activeIndex =
          swiperState.value.swiperInstance.activeIndex * swiperState.value.swiperItemsPerPage +
          newIndex
  ...
  watch(
    () => swiperState.value.changeSwiperIndex,
    debounce(() => {
      swiperBallHandler.getFloatingBallPosition()
    }, 50)
  )
