...
<template>
  <div class="swiper-container">
    <div
      v-if="paginatedClipboardList.length > 0"
      class="floating-ball"
      ref="floatingBall"
    ...
    <Swiper v-bind="swiperServices.params" @swiper="swiperServices.onSwiperInit" ref="swiperRef">
      <SwiperSlide
        v-for="(pageItems, pageIndex) in paginatedClipboardList"
        :key="`page-${pageIndex}`"
      >
        <TransitionGroup name="card-list" tag="div" class="clipboard-list">
          <div
            v-for="(card, cardIndex) in pageItems"
            :key="card.contentHash + 'key' + cardIndex"
            class="card-wrapper"
          >
            <StickyBadge :card="card" :card-id="card.id" />
            <VClipboardCard :clipboardOptions="card" />
